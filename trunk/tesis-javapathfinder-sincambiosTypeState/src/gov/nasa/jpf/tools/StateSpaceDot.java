 ...
 */
public class StateSpaceDot implements SearchListener {
  ...
  private static final int state_node_style =
    DotConstants.ELLIPSE_WITH_TEXT;
  private static final int transition_node_style =
    DotConstants.RECTANGLE_WITH_TEXT;
  ...
  private String getErrorMsg(Search search) {
    ErrorList errs = search.getErrors();
    if (errs.isEmpty()) {
      return null;
    } else {
      return errs.get(0).getMessage();
    }
  }
  ...
  private String makeDotLabel(Search state, int my_id) {
    ...
    if (show_source) {
      String source_line=last_trans_step.getSourceRef().getLineString();
      if ((source_line != null) && !source_line.equals("")) {
        result.append("\\n");

        StringBuffer sb=new StringBuffer(source_line);
   ...
   * some special chars so here we use StringBuffer's replace method to do
   * this.
   ...
   *         throughout.
   */
  private StringBuffer replaceString(StringBuffer original,
  		                             String from,
									 String to) {
  ...
  private String replaceString(String original, String from, String to) {
      ...
      return replaceString(new StringBuffer(original), from, to).toString();
  	} else {
      return original;
  	}
  }
  ...
  private String makeGdfEdgeString(String from_id,
  	...
  	sb.append(from_id).append(',').append(to_id).append(',').
	  append('\"');
   ...
   */
  private void convertGdfSpecial(StringBuffer sb) {
  	replaceString(sb, "\"", "\'\'");
  	replaceString(sb, "\n", " ");
  }
