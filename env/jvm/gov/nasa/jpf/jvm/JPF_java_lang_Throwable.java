//
// Copyright (C) 2006 United States Government as represented by the
// Administrator of the National Aeronautics and Space Administration
// (NASA).  All Rights Reserved.
// 
// This software is distributed under the NASA Open Source Agreement
// (NOSA), version 1.3.  The NOSA has been approved by the Open Source
// Initiative.  See the file NOSA-1.3-JPF at the top of the distribution
// directory tree for the complete NOSA document.
// 
// THE SUBJECT SOFTWARE IS PROVIDED "AS IS" WITHOUT ANY WARRANTY OF ANY
// KIND, EITHER EXPRESSED, IMPLIED, OR STATUTORY, INCLUDING, BUT NOT
// LIMITED TO, ANY WARRANTY THAT THE SUBJECT SOFTWARE WILL CONFORM TO
// SPECIFICATIONS, ANY IMPLIED WARRANTIES OF MERCHANTABILITY, FITNESS FOR
// A PARTICULAR PURPOSE, OR FREEDOM FROM INFRINGEMENT, ANY WARRANTY THAT
// THE SUBJECT SOFTWARE WILL BE ERROR FREE, OR ANY WARRANTY THAT
// DOCUMENTATION, IF PROVIDED, WILL CONFORM TO THE SUBJECT SOFTWARE.
//
package gov.nasa.jpf.jvm;

import java.util.HashMap;


/**
 * MJI NativePeer class for java.lang.Throwable library abstraction
 */
public class JPF_java_lang_Throwable {
  static HashMap<?,?> stackTraces;  // needed? -pcd
  
  /**
   * return array of StackTraceElement elements with the current threads callstack
   */
  public static int getStackTrace_____3Ljava_lang_StackTraceElement_2 (MJIEnv env, int objref) {
    return env.getThreadInfo().getStackTrace(objref);
  }
  
  public static int fillInStackTrace____Ljava_lang_Throwable_2 (MJIEnv env, int objref) {
    int traceRef = getStackTrace_____3Ljava_lang_StackTraceElement_2( env, objref);
    DynamicArea da = env.getDynamicArea();
    /*ElementInfo ei =*/ da.get(objref); // needed?  -pcd
    
    env.setReferenceField(objref, "stackTrace", traceRef);
    return MJIEnv.NULL;
  }
  
  // native because we don't want to waste states
  public static void printStackTrace____V (MJIEnv env, int objRef) {
    
    env.getThreadInfo().printStackTrace(objRef);
  }
}
